...
from .models import db, User, Bio, Blog
...
def get_index():
  ...
  if current_user.is_authenticated and current_user.admin:
    session["admin"] = True
  return render_template("static/about_us.jinja2") 
...
def get_to_know_them(email):
  ...
  return render_template("get_to_know/get_to_know_them.jinja2", bio = bio)
...
@app.route("/get-to-know-me/<email>/request", methods = ["GET", "POST"])
def get_to_know_request():
  form = RequestForm()
...
def blog():
  if request.method == "POST" and current_user.admin:
    ...
    db.session.commit()
    return ""
  return render_template("blog/blog.jinja2", blogs = Blog.query.all())
...
def create():
  ...
  if form.validate_on_submit():
    ...
    blog = Blog(email = current_user.email, name = current_user.name, country = current_user.country, blog = form.blog.data, photo0 = url0, photo1 = url1, photo2 = url2, youtube = form.youtube.data if form.youtube else None)
    db.session.add(blog)
    db.session.commit()
...
def admin():
  if current_user.admin:
    if request.method == "POST":
      ...
      db.session.commit()
      return ""
    return render_template("user/admin.jinja2", users = User.query.order_by(User.admin.desc()).all())
  else:
